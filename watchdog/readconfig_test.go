...
func TestRead_FaasRegistryConfig(t *testing.T) {
	defaults := NewEnvBucket()
	readConfig := ReadConfig{}

	// No map should be created.
	config := readConfig.Read(defaults)
	if config.faasRegistry != nil {
		t.Logf("Default faasRegistry envVariable incorrect, got: \"%s\".\n", config.faasRegistry)
		t.Fail()
	}

	defaults.Setenv("faas_registry", "helloworld")
	config = readConfig.Read(defaults)

	if len(config.faasRegistry) != 1 {
		t.Logf("Single faasRegistry envVariable incorrect, got: \"%s\".\n", len(config.faasRegistry))
		t.Fail()
	}
	if _, registered := config.faasRegistry["helloworld"]; !registered {
		t.Logf("Single faasRegistry for function: unregistered")
		t.Fail()
	} else if config.faasRegistry["helloworld"].pid != 0 {
		t.Logf("Single faasRegistry incorrect value for function: \"%d\".\n", config.faasRegistry["helloworld"])
		t.Fail()
	}

	defaults.Setenv("faas_registry", "helloworld,nodejs")
	config = readConfig.Read(defaults)

	if len(config.faasRegistry) != 2 {
		t.Logf("Multiple faasRegistry envVariable incorrect, got: \"%s\".\n", len(config.faasRegistry))
		t.Fail()
	}
	if _, registered := config.faasRegistry["helloworld"]; !registered {
		t.Logf("Multiple faasRegistry for 1st function: unregistered")
		t.Fail()
	} else if config.faasRegistry["helloworld"].pid != 0 {
		t.Logf("Multiple faasRegistry incorrect value for 1st function: \"%d\".\n", config.faasRegistry["helloworld"].pid)
		t.Fail()
	}
	if _, registered := config.faasRegistry["nodejs"]; !registered {
		t.Logf("Multiple faasRegistry for 2nd function: unregistered")
		t.Fail()
	} else if config.faasRegistry["nodejs"].pid != 0 {
		t.Logf("Multiple faasRegistry incorrect value for 2nd function: \"%d\".\n", config.faasRegistry["nodejs"].pid)
		t.Fail()
	}
}
